...
COPY . .
RUN go build -o robostep-util-server cmd/main.go

FROM alpine:latest
RUN apk --no-cache add ca-certificates
WORKDIR /root/

COPY --from=builder /app/robostep-util-server .
COPY --from=builder /app/.env .

CMD ["./robostep-util-server"]
```

### systemd サービスとして実行する場合

`/etc/systemd/system/robostep-util-server.service`:

```ini
[Unit]
Description=Robostep Util Server
After=network.target

[Service]
Type=simple
User=your-user
WorkingDirectory=/path/to/robostep-util-server
ExecStart=/path/to/robostep-util-server/robostep-util-server
Restart=always
...
1. **Discord Webhook URL が設定されていない**
   - `.env` ファイルに `DISCORD_WEBHOOK_URL` が正しく設定されているか確認

2. **Notion Webhook が動作しない**
   - Notion データベースのWebhook設定を確認
   - サーバーが外部からアクセス可能な状態になっているか確認

3. **JSON パースエラー**
   - Notion からのペイロード形式が想定と異なる場合があります
   - ログを確認してエラー内容を調査
